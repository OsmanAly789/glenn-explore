...
export class VehiclePhysics {
  ...
  private calculatePhysics(deltaTime: number, input: PhysicsInput): Pick<PhysicsResult, 'velocity' | 'acceleration' | 'speed'> {
    ...
    if (input.brake) {
      if (this.velocity > 0.5) {
        ...
        netForce -= this.config.engineForce * 0.6; // Reverse with 60% power
      }
    }
  ...
  private checkCollision(ctx: {
    ...
    const transform = Cesium.Transforms.eastNorthUpToFixedFrame(ctx.position);

    const localForward = new Cesium.Cartesian3(
      Math.cos(ctx.heading),
      -Math.sin(ctx.heading),
      0
    );
